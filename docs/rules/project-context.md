...
- PHP, Laravel, Livewire, and Pest/PHPUnit use the compatible versions locked by project dependencies. Use the active theme proven in source. Scramble documents the optional, disabled-by-default `api.<APP_DOMAIN>` gateway. Session/cache drivers are file and queue defaults to `sync` for shared hosting.
