<div x-data="{ open: false, top: 0, left: 0 }" @click.outside="open = false" @scroll.window="open = false" @resize.window="open = false">
    ...
    <template x-teleport="body">
        ...
        @if (! $row->trashed())
            <li>
                <button type="button" class="dropdown-item" wire:click="$dispatch('starter-role-users-request', { id: {{ $row->id }} })">User</button>
            </li>
            <li>
                <button type="button" class="dropdown-item" wire:click="$dispatch('starter-role-access-request', { id: {{ $row->id }} })">Akses</button>
            </li>
            <li>
                <a class="dropdown-item" href="{{ route('starter.settings.roles.edit', ['roleId' => $row->id]) }}">Edit</a>
            </li>
            @if (! $row->isSuperuser())
