...
test('role create and edit use dedicated pages', function () {
    ...
    $this->actingAs($login)
        ...
        ->assertSeeHtml('wire:model="roleForm.code"')
        ->assertSeeHtml('wire:model="roleForm.name"')
...
test('starter navigation keeps settings query string changes', function () {
    $runtime = file_get_contents(public_path('assets/mine/starter-runtime.js'));
...
test('livewire ajax requests use the global blur loader', function () {
    $login = starterToastLogin();
    $runtime = file_get_contents(public_path('assets/mine/starter-runtime.js'));
    $appLayout = file_get_contents(resource_path('views/templates/layouts/app.blade.php'));
    $authLayout = file_get_contents(resource_path('views/templates/layouts/auth.blade.php'));
    ...
    Livewire::test(ClientProfile::class)
        ...
        ->assertDispatched('starter-client-branding-updated', function (string $event, array $params): bool {
            return $event === 'starter-client-branding-updated'
                && str_ends_with((string) ($params['logoUrl'] ?? ''), '/assets/mine/client-logo.png')
...
test('module accordion toggles first and second app from livewire state only', function () {
    $login = starterToastLogin();
    ...
    Livewire::test(RoleForm::class)
        ...
        ->assertDontSeeHtml('id="role-app-modules-app-'.$appOne->id.'" class="accordion-collapse collapse show"')
        ->call('toggleModuleApp', 'app-'.$appTwo->id)
        ->assertSet('expandedModuleAppKeys', ['app-'.$appTwo->id])
